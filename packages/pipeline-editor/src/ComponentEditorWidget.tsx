 ...
 */
export class ComponentEditorWidget extends ReactWidget {
  props: IComponentEditorProps;
  widgetClass: string;
  schema: any = {};
  metadata: any = {};
  loading = true;
  ...
  constructor(props: IComponentEditorProps) {
    super();
    this.props = props;
    this.widgetClass = `elyra-metadataEditor-${props.path.substring(
      props.path.lastIndexOf('/') + 1
    )}`;
  ...
  async loadSchemaAndMetadata(): Promise<void> {
    try {
      ...
      this.title.label = `${this.schema.title}`;
      this.loading = false;
      this.update();
    } catch (error) {
