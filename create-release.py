...
def update_version_to_release() -> None:
    ...
    try:
        ...
        sed(
            _source("etc/docker/kubeflow/README.md"),
            r"kf-notebook:dev",
        ...
        sed(
            _source("docs/source/recipes/configure-airflow-as-a-runtime.md"),
            r"main",
            f"{config.tag}",
        )
        sed(
...
def checkout_code() -> None:
    ...
    check_run(
        ["git", "clone", config.git_url, "-b", config.git_branch],
        cwd=config.work_dir,
    )
    check_run(
...
def build_release():
    ...
    if not config.pre_release:
        ...
        check_run(
            ["git", "checkout", f"tags/v{config.new_version}"],
            cwd=config.source_dir,
            capture_output=False,
        )
        check_run(
...
def prepare_extensions_release() -> None:
    ...
    for extension in extensions:
        ...
        check_run(
            ["cp", _source("etc/templates/setup.py"), extension_source_dir],
            cwd=config.work_dir,
        )
        # update template
        ...
        sed(
            setup_file,
            "{{data - files}}",
            re.escape("('share/jupyter/labextensions', 'dist/labextensions', '**')"),
        )
        sed(
            setup_file,
            "{{install - requires}}",
            f"'elyra-server=={config.new_version}',",
        )
        sed(
            setup_file,
            "{{description}}",
            f"'{extensions[extension].description}'",
        )
...
def prepare_runtime_extensions_package_release() -> None:
    ...
    for package in packages:
        ...
        check_run(
            ["cp", _source("etc/templates/setup.py"), package_source_dir],
            cwd=config.work_dir,
        )
        # update template
...
def prepare_changelog() -> None:
    ...
    check_run(
        [
            ...
            "-a",
            "-m",
            f"Update changelog for release {config.new_version}",
        ],
        cwd=config.source_dir,
    )
...
def prepare_release() -> None:
    ...
    check_run(
        [
            ...
            "-a",
            "-m",
            f"Prepare for next development iteration",
        ],
        cwd=config.source_dir,
    )
    # prepare extensions
...
def main(args=None):
    ...
    parser.add_argument(
        "--dev-version",
        help="the new development version",
        type=str,
        required=False,
    )
    ...
    parser.add_argument(
        "--org",
        help="the github org or username to use",
        type=str,
        required=False,
    )
