...
class FileOpBase(ABC):
    ...
    def process_metrics_and_metadata(self) -> None:
        ...
        fd = os.open(ui_metadata_output, os.O_WRONLY | os.O_CREAT | os.O_TRUNC)
        with os.fdopen(fd, "w") as f:
            json.dump(metadata, f)
        duration = time.time() - t0
...
class NotebookFileOp(FileOpBase):
    ...
    def convert_notebook_to_html(notebook_file: str, html_file: str) -> str:
        ...
        fd = os.open(html_file, os.O_WRONLY | os.O_CREAT | os.O_TRUNC)
        with os.fdopen(fd, "w") as f:
            f.write(data)
...
class PythonFileOp(FileOpBase):
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            t0 = time.time()
            fd = os.open(python_script_output, os.O_WRONLY | os.O_CREAT | os.O_TRUNC)
...
class RFileOp(FileOpBase):
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            t0 = time.time()
            fd = os.open(r_script_output, os.O_WRONLY | os.O_CREAT | os.O_TRUNC)
            with os.fdopen(fd, "w") as log_file:
                subprocess.run(
                    ["Rscript", r_script],
                    stdout=log_file,
